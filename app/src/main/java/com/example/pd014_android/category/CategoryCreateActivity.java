...
import com.example.pd014_android.BaseActivity;
...
public class CategoryCreateActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtImageError=findViewById(R.id.textImageError);
    ...
    public void handleCreateCategoryClick(View view) {
        if(!validation()) {
            Toast.makeText(this, "Не коректно вказано дані!", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    private boolean validation() {
        ...
        if(uri==null) {
            txtImageError.setVisibility(View.VISIBLE);
            isValid=false;
        }
        return isValid;
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(resultCode==SELECT_CROPPER) {
            ...
            txtImageError.setVisibility(View.INVISIBLE);
